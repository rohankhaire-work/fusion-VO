...
namespace imu_measurement
{
  ...
  quaternion_derivative(const Eigen::Quaterniond &q, const Eigen::Vector3d &omega)
  {
    ...
    return Eigen::Quaterniond(0.5 * q.coeffs()) * omega_quat;
  }
                               ...
                               const Eigen::Vector3d &gyro, double dt)
  {
    ...
    Eigen::Quaterniond q2
      = state.orientation
        * Eigen::Quaterniond(1, 0.5 * k1_q.x() * dt, 0.5 * k1_q.y() * dt,
                             0.5 * k1_q.z() * dt);
    ...
    Eigen::Quaterniond q3
      = state.orientation
        * Eigen::Quaterniond(1, 0.5 * k2_q.x() * dt, 0.5 * k2_q.y() * dt,
                             0.5 * k2_q.z() * dt);
    ...
    Eigen::Quaterniond q4
      = state.orientation
        * Eigen::Quaterniond(1, k3_q.x() * dt, k3_q.y() * dt, k3_q.z() * dt);
